...
class MarginfiAccountWrapper {
  ...
  public async makeEndFlashLoanIx(override_banks: 
    Map<string, Bank>): Promise<InstructionsWrapper> {
      let banks = this.client.banks;
      if (override_banks) {
        for (const [key, value] of override_banks.entries()) {
          banks.set(key, value);
    ...
    return this._marginfiAccount.makeEndFlashLoanIx(this._program, banks, override_banks)
  }

  public async flashLoan(args: FlashLoanArgs, override_banks :
    Map<string, Bank> ): Promise<string> {
    ...
    const endFlashLoanIx = await this.makeEndFlashLoanIx(override_banks);
    ...
    const sig = await this.client.processTransaction(tx, args.signers, {skipPreflight: true});
